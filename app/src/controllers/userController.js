...
exports.addUserBiography = async (req, res) => {
  try {
    ...
    if (user.biography) {
      return res.status(400).json({ success: false, message: 'User biography already exists.' });
    }

    // Adiciona a biografia ao usuário
    user.biography = biography;
    await user.save();
    return res.status(201).json({ success: true, message: 'User biography added successfully.' });
  } catch (error) {
    console.error('Error adding user biography:', error);
    res.status(500).json({ success: false, message: 'Internal server error while adding user biography.' });
  }
};
...
exports.updateUserBiography = async (req, res) => {
  try {
    ...
    user.biography = biography;
    await user.save();
    return res.status(200).json({ success: true, message: 'User biography updated successfully.' });
  } catch (error) {
    console.error('Error updating user biography:', error);
    res.status(500).json({ success: false, message: 'Internal server error while updating user biography.' });
  }
};
...
exports.addUserProfileImage = async (req, res) => {
  try {
    ...
    if (user.profileImageUrl) {
      return res.status(400).json({ success: false, message: 'User profile image already exists.' });
    }
    ...
    return res.status(201).json({ success: true, message: 'User profile image added successfully.' });
  } catch (error) {
    console.error('Error adding user profile image:', error);
    return res.status(500).json({ success: false, message: 'Internal server error while adding user profile image.' });
  }
};
...
exports.updateUserProfile = async (req, res) => {
  try {
    ...
    if (newPhone) {
      ...
      if (existingPhone) {
        return res.status(402).json({ error: "O número de telefone já está em uso." });
      }
      user.phone = newPhone;
    }
    ...
    return res.status(500).json({ success: false, message: 'Internal server error while updating user profile.' });
  }
};
...
exports.deleteUserProfileImage = async (req, res) => {
  try {
    ...
    if (!user.profileImageUrl) {
      return res.status(400).json({ success: false, message: 'User profile image does not exist.' });
    }
    ...
    return res.status(200).json({ success: true, message: 'User profile image deleted successfully.' });
  } catch (error) {
    console.error('Error deleting user profile image:', error);
    return res.status(500).json({ success: false, message: 'Internal server error while deleting user profile image.' });
  }
};
...
exports.addGalleryImage = async (req, res) => {
  try {
    ...
    user.galleryImageUrl.push(galleryImageUrl);
      ...
      return res.status(201).json({ success: true, message: 'Gallery image added successfully.' });
    } catch (error) {
      console.error('Error adding gallery image:', error);
      return res.status(500).json({ success: false, message: 'Internal server error while adding gallery image.' });
    }
  }  
  ...
  exports.getGalleryImages = async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, galleryImageUrls: user.galleryImageUrl });
    } catch (error) {
      console.error('Error getting gallery images:', error);
      return res.status(500).json({ success: false, message: 'Internal server error while getting gallery images.' });
    }
  };
