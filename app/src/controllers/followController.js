...
exports.unfollowUser = async (req, res) => {
    ...
    try {
        ...
        const deletedRelationship = await Relationship.findOneAndDelete({ follower_id, following_id });

        if (!deletedRelationship) {
            return res.status(404).json({ error: "Relacionamento de seguidor não encontrado" });
        }
...
async function updateUserFollowersCount(userId) {
    try {
        // Encontrar o usuário pelo ID
        const user = await User.findById(userId);

        // Atualizar o contador de seguidores
        user.followersCount = await Relationship.countDocuments({ following_id: userId });
        
        // Salvar as alterações no banco de dados
        await user.save();
    } catch (error) {
        console.error("Erro ao atualizar o contador de seguidores do usuário:", error);
    }
}