...
function LoginForm() {
  ...
  const checkFieldAvailability = async (fieldName, value, t) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error checking availability of ${fieldName}: ${response.statusText}`
        );
      }

      const data = await response.json();
      return data; // Deve conter uma propriedade "available" indicando se está disponível ou não
    } catch (error) {
      console.error(`Error checking availability of ${fieldName}:`, error);
